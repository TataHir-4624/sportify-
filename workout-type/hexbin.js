...
// === HOVER INFO (POSITIONED NEAR DOT) ===
circles
  .on("mouseover", function (event, d) {
    // Get mouse position relative to the page
    const tooltipX = event.pageX + 5;  // 15px to the right of cursor
    const tooltipY = event.pageY + 5;  // 15px below cursor
...
d3.selectAll(".button-wrap").on("click", function() {
  ...
  // If zoomed in, zoom out first then highlight
  if (zoomedIn) {
    zoomedIn = false;
    activeHex = null;

    // Remove dots and hide song info
    songLayer.selectAll("circle")
      .transition().duration(400)
      .attr("r", 0)
      .remove();

    d3.select("#song-info-fixed").style("opacity", 0);

    // Zoom out, then highlight after animation finishes
    g.transition().duration(800)
      .attr("transform", `translate(${margin.left},${margin.top}) scale(1)`)
      .on("end", () => highlightWorkout(type));
