...
function centerColumn(layer) {
    ...
    const targetScroll = midpointScreen - (500 / 2.7);
...
document.querySelectorAll('.genre-label').forEach(button => {
    ...
    button.addEventListener('click', function() {
        ...
        if (genreNode) {
            ...
            // Show next button after selection is made
            document.getElementById('next-button').style.display = 'flex';
        }
    });
});
...
window.filterAndShowSongs = function() {
    ...
    songDiv.innerHTML = `
        <div class="song-title" style="max-width: 250px; word-wrap: break-word; line-height: 1.3;">${song.title}</div>
        <div class="song-details">
            Artist: ${song.artist} | BPM: ${song.bpm} | Energy: ${song.nrgy}%
        </div>
        <div class="song-details">
            Valence: ${song.val}% | Year: ${song.year}
        </div>
    `;
